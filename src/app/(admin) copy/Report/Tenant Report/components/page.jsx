...
const TenantAvatar = ({ item }) => {
  ...
  return (
    <div className={`${getAvatarColor(item.firstName)} rounded-circle d-flex align-items-center justify-content-center`} style={{ width: '32px', height: '32px', fontWeight: 600 }}>
...
const TenantReportsPage = () => {
  ...
  const [tenantType, setTenantType]       = useState({ active: true, inactive: true });
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res  = await httpClient.get(`${API_BASE_URL}/helper/country/get_all?limit=999999`, { headers: HEADERS });
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res  = await httpClient.get(`${API_BASE_URL}/helper/city/get_all?limit=999999`, { headers: HEADERS });
        const data = res.data?.data?.data ?? res.data?.data ?? [];
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true);
    try {
      const url = `${API_BASE_URL}/lead/get_all/?limit=999999`;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, fromDate, toDate, selectedCity, selectedCountry, tenantType, selectedOrigin]);

  const filteredData = useMemo(() => {
    return rawData.filter((lead) => {
      const purpose = String(lead.purpose || '').toLowerCase();
      if (!TENANT_PURPOSES.includes(purpose)) return false;
      ...
      const statusMatch = (tenantType.active && lead.isActive) || (tenantType.inactive && !lead.isActive);
  ...
  }, [rawData, searchQuery, selectedCity, tenantType, selectedOrigin, fromDate, toDate]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Tenant_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
          ...
          <Card className="border-0 shadow-sm">
            <CardBody className="p-3">
              <Row className="align-items-center g-3">
                ...
                <Col lg={2}>
                  <h5 className="mb-0">{filteredData.length} <span className="text-muted fw-normal">Tenants</span></h5>
                </Col>
                <Col lg={7}>
        ...
        <Col xl={3} lg={4}>
          <Card className="border-0 shadow-sm mb-4">
            ...
            <CardBody>
              ...
              {/* Tenants Rent */}
              <div className="mb-4">
                <label className="form-label fw-bold mb-2">Tenants Rent :</label>
                <div className="d-flex gap-3">
                  <div className="form-check"><input className="form-check-input" type="checkbox" id="rp" /><label className="form-check-label text-muted small" htmlFor="rp">Rent Paid</label></div>
                  <div className="form-check"><input className="form-check-input" type="checkbox" id="rpen" /><label className="form-check-label text-muted small" htmlFor="rpen">Rent Pending</label></div>
                </div>
              </div>

              {/* Tenant Type */}
              <div className="mb-4">
                <label className="form-label fw-bold small">Tenant Type :</label>
                <div className="d-flex flex-column gap-2">
                  <div className="form-check">
                    <input className="form-check-input" type="checkbox" checked={tenantType.active} onChange={() => setTenantType({ ...tenantType, active: !tenantType.active })} id="act" />
                    <label className="form-check-label text-muted small" htmlFor="act">Active</label>
                  </div>
                  <div className="form-check">
                    <input className="form-check-input" type="checkbox" checked={tenantType.inactive} onChange={() => setTenantType({ ...tenantType, inactive: !tenantType.inactive })} id="inact" />
                    <label className="form-check-label text-muted small" htmlFor="inact">Inactive Tenant</label>
                  </div>
                </div>
              </div>
              ...
              <div className="table-responsive">
                <table className="table align-middle mb-0 text-nowrap table-hover">
                  <thead className="bg-light">
                    <tr>
                      <th className="ps-3 small text-uppercase text-muted">Tenant ID</th>
                      <th className="small text-uppercase text-muted">Tenant Photo & Name</th>
                  ...
                  <tbody>
                    {loading ? (
                          ...
                          <td>
                            <div className="d-flex gap-2">
                              <Button variant="light" size="sm" title="View details" onClick={() => navigate('/lead/preview', { state: { lead: item } })}>
                                <IconifyIcon icon="solar:eye-broken" width={18} />
                              </Button>
                            </div>
                          </td>
                        </tr>
                      ...
                      <tr><td colSpan="8" className="text-center py-4 text-muted">No tenants found matching these filters.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
